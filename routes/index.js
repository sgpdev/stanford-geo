...
router.get("/get", function(req, res) {
    ...
    if (req.body.attribute) {
        if (search_atts[req.body.attribute]) {
            ...
            res
                .status(400)
                .send(
                    `attribute error: ${req.body.attribute} is not a valid attribute in this API call.`
                );
            return;
        }
    }
    ...
    if (req.body.current_search && typeof req.body.current_search === "string") {
        ...
        res
            .status(400)
            .send(
                `current_search error: ${req.body.current_search} is not a valid in this API call.`
            );
        return;
    }
    ...
    // Construct SQL and query database for results
    var query = `SELECT DISTINCT ${cur_att.attribute_db} FROM ${cur_table} WHERE ${cur_att.attribute_db} IS NOT NULL AND ${cur_att.attribute_db} LIKE '${cur_search}%' ORDER BY ${cur_att.attribute_db} LIMIT ${cur_limit};`;
